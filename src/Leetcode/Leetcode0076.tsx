...
export default function Leetcode0076() {
  ...
  React.useEffect(() => {
    ...
    const movingBagNumbers = getChildren<SVGRectElement>(movingBagControl);
    for (let i = 0; i < 3; i++) {
      movingBagNumbers[i].textContent = `${frame.movingBag[i]}`;
    }

    // moving-bag-rects
    const movingBagRectControl = getElementById('moving-bag-rects');
    const movingBagRects = getChildren<SVGRectElement>(movingBagRectControl);
    for (let i = 0; i < 3; i++) {
  ...
  return (
    <>
      <svg
        ...
        <text x='400' y='260' fill='black' style={titleStyle}>
          Has?
        </text>
        <text
          id='superset'
